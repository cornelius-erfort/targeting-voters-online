...
# source: https://economictheoryblog.com/2016/12/13/clustered-standard-errors-in-r/

summary.cluster <- function (object, correlation = FALSE, 
...
# source: https://rdrr.io/github/timvdvalk/starnote/src/R/starnote.R
starnote <- function(dta = "", save_loc = "", tablenote = "", colnumber = 6, width_col1 = 0.15, caption = ""){
  nx <- paste(replicate(colnumber, "s"), collapse = "")
  nc <- paste(replicate(colnumber, "t"), collapse = "")
  dta %>% str_c(collapse = "\n") %>%
    str_replace("begin\\{tabular\\}",
                "begin\\{tabularx\\}") %>%
    str_replace("end\\{tabular\\}", "end\\{tabularx\\}") %>% str_replace("...extracolsep.*lc*.", str_c("\\\\textwidth\\}{t", rep("s", colnumber - 1) %>% str_c(collapse = ""), "\\}")) %>% 
    str_replace("4..r", "2\\}\\{r") %>% str_replace("\\\\caption\\{", str_c("\\\\caption\\{", caption)) %>% 
    str_remove("; \\$\\^\\{\\*{3}.*\\]") %>%
    cat(file = save_loc)
}

clust <- function (c) summary.cluster(c, cluster = "parlgov_id")$coefficients[, 2] %>% as.numeric()
